...
function App() {
    ...
    }, [rainValue, simMode]); // keeping your deps as-is
    ...
    const predictRisk = async (latlng, manualRainOverride = null) => {
        ...
        try {
            const rainToSend =
                simMode && manualRainOverride !== null
                    ? manualRainOverride
                    : simMode
                    ? rainValue
                    : null;
            ...
            const response = await axios.post(
                'https://landslide-detector-backend.vercel.app/predict',
                {
                    lat: latlng.lat,
                    lng: latlng.lng,
    ...
    return (
        // ⭐ CHANGED: use flex layout, stack on mobile, side-by-side on md+
        <div className="h-screen w-screen bg-slate-900 font-sans text-slate-800 flex flex-col md:flex-row">
            {/* ⭐ CHANGED: responsive control panel container */}
            <div
                className="
                    relative z-[1000]
                    w-full md:w-96
            ...
            >
                <div className="bg-white/95 backdrop-blur-md p-5 rounded-2xl shadow-xl border-l-8 border-cyan-500">
                    <h1 className="text-2xl font-extrabold text-slate-800">
                        LAP <span className="text-cyan-500">SUS</span>
                    </h1>
                    <p className="text-xs text-slate-500 mt-1">
                        Real-time Landslide Analysis System
                    </p>
                </div>
                ...
                <div className="bg-white/95 backdrop-blur-md p-4 rounded-xl shadow-lg space-y-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-1">
                            Approximate depth of potential failure plane.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden transition-all duration-500">
                        <div
                            className={`p-5 text-white flex justify-between items-center ${
                                result.prediction.level === 'High'
                                    ? 'bg-red-600'
                                    : result.prediction.level === 'Medium'
                                    ? 'bg-orange-500'
                                    : 'bg-green-600'
                            }`}
                        >
                            <div>
                                ...
                                <p className="text-xs opacity-90 mt-1">
                                    {getEnvironmentIcon(result.prediction.environment)}{' '}
                                    {result.prediction.environment}
                                </p>
                            </div>
                            <div className="text-right">
                                <p className="text-xs opacity-80">Safety Factor</p>
                                <p className="text-2xl font-mono">
                                    {result.prediction.details.FoS.toFixed(2)}
                                </p>
                                <p className="text-[10px] opacity-70 mt-1">
                                    {result.prediction.details.FoS < 1
                                        ? 'FAILURE'
                                        : result.prediction.details.FoS < 1.5
                                        ? 'UNSTABLE'
                                        : 'STABLE'}
                                </p>
                            </div>
                        ...
                        <div className="p-5 space-y-4 max-h-[50vh] overflow-y-auto">
                            ...
                            {result.prediction.soil_type && (
                                <div className="bg-amber-50 p-3 rounded-lg border border-amber-200">
                                    ...
                                    <p className="text-xs text-amber-600 mt-1">
                                        Clay: {result.data.clay.toFixed(0)}% | Sand:{' '}
                                        {result.data.sand.toFixed(0)}% | Silt:{' '}
                                        {result.data.silt.toFixed(0)}%
                                    </p>
                                </div>
                            )}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <StatBox
                                        label="Elevation"
                                        value={`${Math.round(result.data.elevation)}m`}
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <StatBox
                                        label="Cohesion (c)"
                                        value={`${result.prediction.details.cohesion} kPa`}
                                    />
                                    <StatBox
                                        label="Friction (φ)"
                                        value={getFrictionDisplay(result)}
                                    />
                                    <StatBox
                                    ...
                                    <StatBox
                                        ...
                                        warning={
                                            parseFloat(
                                                result.prediction.details.shear_stress
                                            ) >
                                            parseFloat(
                                                result.prediction.details.shear_strength
                                            )
                                        }
                                    />
                                </div>
                                {result.prediction.details.depth && (
                                    <p className="text-[10px] text-slate-500 mt-1">
                                        Failure depth used:{' '}
                                        {result.prediction.details.depth} m
                                    </p>
                                )}
                            </div>
            ...
            {/* ⭐ CHANGED: map gets remaining space, fixed height on mobile */}
            <div className="flex-1 h-full md:h-auto">
                <MapContainer
                    center={[10.8505, 76.2711]}
                    zoom={8}
                    scrollWheelZoom={true}
                    className="h-[45vh] md:h-full w-full z-0"
                >
