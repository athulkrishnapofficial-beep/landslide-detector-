...
function App() {
    ...
    // 💡 NEW STATE: To control the visibility of the control panel on mobile
    ...
    const predictRisk = async (latlng, manualRainOverride = null) => {
        ...
        try {
            ...
            // 💡 NEW: Open the panel automatically on successful prediction on mobile
    ...
    return (
        <div className="relative h-screen w-screen bg-slate-900 font-sans text-slate-800">
            ...
            {/* Visible on small screens, fixed at bottom-right or top-left (easier for user access if panel is at bottom) */}
            <button
                onClick={() => setIsPanelOpen(!isPanelOpen)}
                className="md:hidden absolute bottom-4 right-4 z-[1000] p-3 rounded-full bg-cyan-600 text-white shadow-xl hover:bg-cyan-700 transition-colors"
            >
                {isPanelOpen ? <X size={24} /> : <Menu size={24} />}
            </button>
            
            {/* 💡 CONTROL/RESULT PANEL - MOBILE & DESKTOP LAYOUTS */}
            <div 
                className={`
                    absolute z-[1000] w-full p-4 
                    // Mobile (sm-md): fixed at bottom, slide-up/down
                    bottom-0 left-0 
                    transition-transform duration-300 ease-in-out 
                    ${isPanelOpen ? 'translate-y-0' : 'translate-y-full'} 
                    max-h-[80vh] overflow-y-auto

                    // Desktop (md+): fixed at top-left, full height sidebar
                    md:top-4 md:left-4 md:translate-y-0 
                    md:w-80 lg:w-96 md:max-h-[calc(100vh-2rem)] md:p-0 md:pr-4
                `}
            >
                {/* 💡 Mobile Handle Bar/Title - to be fixed for easy dragging/closing */}
                <div 
                    className="md:hidden bg-white/95 backdrop-blur-md px-5 pt-3 pb-3 rounded-t-2xl shadow-xl border-t-8 border-cyan-500 cursor-pointer flex justify-between items-center"
                    onClick={() => setIsPanelOpen(!isPanelOpen)}
                >
                    <h1 className="text-xl font-extrabold text-slate-800">LAP <span className="text-cyan-500">SUS</span> Analysis</h1>
                    {isPanelOpen ? <ChevronUp size={20} className='text-slate-500' /> : <Menu size={20} className='text-slate-500' />}
                </div>

                {/* 💡 Main Content Wrapper - adjust margin/padding for desktop view */}
                <div className="md:space-y-4 space-y-3 md:pt-0 pb-16 md:pb-0">

                    {/* Header - Hidden on mobile if the handle bar is used as title */}
                    <div className="hidden md:block bg-white/95 backdrop-blur-md p-5 rounded-2xl shadow-xl border-l-8 border-cyan-500">
...
// StatBox component is unchanged and still useful
